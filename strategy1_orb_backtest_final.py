...
# ========= FINAL OPTIMIZED ORB STRATEGY PARAMETERS =========
# Based on research findings and backtest optimization

# Opening Range Parameters (RESEARCH OPTIMIZED)
...
# Breakout Parameters (RESEARCH OPTIMIZED)
...
MIN_BREAKOUT_DISTANCE = 0.3     # Reduced minimum points for valid breakout (research shows 0.5-1.0)

# Risk Management (RESEARCH OPTIMIZED)
STOP_LOSS_ATR_MULTIPLIER = 1.5  # Research shows 1.2-1.8 optimal
TAKE_PROFIT_RATIO = 2.0         # Research shows 2:1 minimum, 2.5:1 better
POSITION_SIZE = 1000            # Position size in dollars
MAX_DAILY_TRADES = 2            # Research shows fewer trades = better quality
MAX_CONCURRENT_POSITIONS = 1    # Maximum concurrent positions

# Time Filters (RESEARCH OPTIMIZED)
...
AVOID_FIRST_MINUTES = 3         # Reduced to 3 minutes after ORB (research shows 5-10 min)
...
# Additional Filters (RESEARCH OPTIMIZED)
USE_VOLUME_FILTER = True        # Enable volume confirmation (research shows this helps)
MIN_VOLUME_MULTIPLIER = 1.1     # Minimum volume must be 1.1x average (research shows 1.2x)
USE_TREND_FILTER = True         # Enable trend confirmation (research shows this helps)
TREND_MA_PERIOD = 20            # Moving average period for trend filter
MIN_TREND_STRENGTH = 0.02       # Minimum trend strength (0.02 = 2% above/below MA)

# ========= FINAL OPTIMIZED ORB STRATEGY CLASSES =========

class FinalORBStrategy:
    """Final Optimized Opening Range Breakout Strategy Implementation"""
    ...
    def check_volume_confirmation(self, df: pd.DataFrame, current_idx: int) -> bool:
        ...
        recent_volume = df.iloc[current_idx-lookback:current_idx]['volume'].mean()
        current_volume = df.iloc[current_idx]['volume']
        
        return current_volume >= (recent_volume * self.params['min_volume_multiplier'])
    
    def check_trend_confirmation(self, df: pd.DataFrame, current_idx: int, direction: str) -> bool:
        """Check if trade direction aligns with trend"""
        if not self.params['use_trend_filter']:
            return True
            
        # Calculate moving average
        ma_period = self.params['trend_ma_period']
        if current_idx < ma_period:
            return True
            
        ma = df.iloc[current_idx-ma_period:current_idx]['close'].mean()
    ...
    def check_breakout(self, df: pd.DataFrame, current_idx: int, range_high: float, range_low: float) -> str:
        """
        Check if current bar represents a valid breakout with additional filters
        ...
        if current_bar['close'] > range_high:
            ...
            if breakout_distance >= self.params['min_breakout_distance']:
                ...
                if (self.check_volume_confirmation(df, current_idx) and 
                    self.check_trend_confirmation(df, current_idx, 'long')):
                    return 'long'
        ...
        # Check for short breakout (below range low)
            ...
            if breakout_distance >= self.params['min_breakout_distance']:
                # Apply additional filters
                if (self.check_volume_confirmation(df, current_idx) and 
                    self.check_trend_confirmation(df, current_idx, 'short')):
                    return 'short'
...
class FinalORBBacktester:
    """Final optimized backtesting engine for ORB strategy"""
    
    def __init__(self, strategy: FinalORBStrategy):
        self.strategy = strategy
        self.results = {}
        
    def run_backtest(self, df: pd.DataFrame) -> Dict:
        """Run the final optimized backtest on historical data"""
        print("Starting Final Optimized ORB Backtest...")
    ...
    def print_results(self):
        ...
        print("\n" + "="*60)
        print("FINAL OPTIMIZED ORB STRATEGY BACKTEST RESULTS")
        print("="*60)
...
def main():
    ...
    # Create final optimized strategy parameters dictionary
    strategy_params = {
    ...
    strategy = FinalORBStrategy(strategy_params)
    backtester = FinalORBBacktester(strategy)
